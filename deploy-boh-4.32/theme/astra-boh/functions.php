...
add_action('wp_enqueue_scripts', function () {
    wp_enqueue_style('boh-style', get_stylesheet_uri(), [], '4.70');
});
...
add_shortcode('boh_rsvp', function ($atts) {
    ...
    $taken = max(0, BOH_TOTAL_SPOTS - BOH_SPOTS_LEFT);
    // Dot row: show last 12 spots
    $window = 12;
    $taken_dots = max(0, $window - BOH_SPOTS_LEFT);
    $taken_dots = min($window, $taken_dots);

    ob_start(); ?>
    <div class="boh-rsvp">
      <div class="boh-rsvp__header">
        <h3 class="boh-rsvp__title">Save your seat</h3>
        <div class="boh-spots">
          <div class="boh-spots__dots" aria-hidden="true">
            <?php for ($i = 0; $i < $window; $i++): ?>
              <span class="boh-spots__dot<?php echo $i < $taken_dots ? ' is-taken' : ''; ?>"></span>
            <?php endfor; ?>
          </div>
          <span class="boh-spots__label"><?php echo (int)BOH_SPOTS_LEFT; ?> spots left</span>
        </div>
      </div>
      <?php
...
// Inject sticky bar on every page (except donate / event where the page is the CTA)
add_action('wp_footer', function () {
    if (is_page(['donate', 'rsvp'])) return;
    echo do_shortcode('[boh_sticky]');
});
...
add_action('wp_footer', function () {
    ...
    (function () {
      ...
      document.querySelectorAll('.boh-steps, .boh-stats').forEach(el => {
      ...
      // Threshold is the height of the hero minus the header — once the hero
      // is mostly past, header becomes opaque + dark text.
      let scrolled = false;
      const hero = document.querySelector('.entry-content > .wp-block-cover:first-child');
      function getThreshold() {
        return hero ? Math.max(80, hero.offsetHeight - 120) : 80;
      }
      function onScroll() {
        const isScrolled = window.scrollY > getThreshold();
...
add_shortcode('boh_hero_slideshow', function () {
    ...
    $slides = [
        ...
        '/wp-content/uploads/2026/07/boh-flower-pattern.jpg',
        '/wp-content/uploads/2026/06/RC_108462_BasketsofHopeEvent-75-1024x681.jpg',
        '/wp-content/uploads/2026/06/RC_BoH_112722_181108_LR-13-1024x683.jpg',
        '/wp-content/uploads/2026/06/RC_BasketofHope_111123_Inital_20171108-15-1024x681.jpg',
    ];
    ob_start(); ?>
    ...
    <script>
    (function () {
      // Reparent the slideshow + dots so they're direct children of the
      // hero cover block. Otherwise they position relative to the block's
      // inner container (which sizes to its text content) and end up
      // outside the visible hero area.
...
// --- [boh_how_it_works] — 4-step process module ------------------------
add_shortcode('boh_how_it_works', function () {
    $steps = [
